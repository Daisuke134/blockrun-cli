...
- REQ-135: WHEN the real 402-quoted price exceeds the local per-second estimate, THE SYSTEM SHALL re-check the budget cap against the real quote BEFORE signing payment (mirrors `video.ts:183-196`). THE SYSTEM SHALL surface the exact quoted amount to the caller (human-readable and `--json`) BEFORE signing in every invocation, not only when a `--budget-limit` was passed — this is the quote-gate that REQ-VER-VIDEO (verification-architecture.md §4.3) relies on to bound real-money risk without any blind-signing step.
