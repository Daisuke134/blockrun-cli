...
- REQ-023: An ergonomic CLI convenience flag that does NOT correspond 1:1 to a source zod field name (e.g. `search`'s documented `query`/`sources`/`max_results`/`from_date`/`to_date` body-shape convention, `exa`'s per-path body-shape conventions) SHALL be implemented ONLY as an explicitly-labeled ALIAS that compiles into the canonical field (`body`, for the path-based passthrough tools) — never as a silent replacement for the canonical field, and never accepted in silent conflict with an explicit `--body` setting the same field (THE SYSTEM SHALL reject that combination with a conflict error). Each command section in §2 that declares such aliases SHALL state the alias→canonical mapping table explicitly, and BOTH the canonical form (`--body '{...}'`) and every alias form SHALL be required to produce an IDENTICAL request object, verified by tests covering both forms (Tier 1: pure `args/<command>.ts` mapping equivalence; Tier 2: mocked-SDK call-shape equivalence) per verification-architecture.md.
...
- REQ-123: `blockrun image <prompt> [--action <generate|edit>] [--model <enum>] [--image <path|url|dataURI>...] [--mask <path|url|dataURI>] [--size <string>] [--quality <standard|hd>] [--inline] [--agent-id <string>]` (`agent_id` is a canonical field per `image.ts` `inputSchema`, REQ-022).
...
- REQ-130: `blockrun video <prompt> [--image-url <url>] [--real-face-asset-id <ta_xxxx>] [--duration-seconds <1-60>] [--generate-audio] [--resolution <enum>] [--aspect-ratio <enum>] [--last-frame-url <url>] [--model <enum>] [--agent-id <string>] [--max-quote-usd <number>]` (`agent_id` is a canonical field per `video.ts` `inputSchema`, REQ-022; `--max-quote-usd` is a CLI-only safety flag, NOT a source field — see REQ-135a).
...
- REQ-144: `blockrun music <prompt> [--instrumental] [--lyrics <string>] [--model <minimax/music-2.5+|minimax/music-2.5>] [--agent-id <string>]` — `--instrumental` defaults true; `agent_id` is a canonical field per `music.ts` `inputSchema`, REQ-022.
...
- REQ-148: `blockrun speech [<input>] [--action <speak|sound_effect|voices>] [--voice <alias|id>] [--model <enum>] [--response-format <mp3|opus|pcm|wav>] [--speed <0.7-1.2>] [--duration-seconds <0.5-22>] [--prompt-influence <0-1>] [--agent-id <string>]` — `--action` default `speak`, `--model` default `elevenlabs/flash-v2.5`, `--response-format` default `mp3`; `agent_id` is a canonical field per `speech.ts` `inputSchema`, REQ-022.
