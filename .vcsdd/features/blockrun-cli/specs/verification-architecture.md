...
| `video` | PROP-113 — per-second cost table + image-input tier switch + mutual-exclusion validation (real_face_asset_id/image_url/last_frame_url) | PROP-114 — subprocess against a **local stub server** replaying a real captured 402→submit→202→poll→completed sequence (fixture JSON), asserting no charge is recorded on a simulated `failed` status | PROP-115 — **quote-gated real call** (decided rule, resolves the §4.3 contradiction — see §4.3 for the full procedure): request `xai/grok-imagine-video`, `--duration-seconds 1` (schema-valid per REQ-130's `1-60` range), `--resolution 360p`. Inspect the 402 quote BEFORE signing (REQ-135's mandatory quote-surfacing). If the quoted amount is `≤ $0.10`, sign and proceed to completion. If higher, do NOT sign; record the quote verbatim in `VERIFICATION.md`; only THEN fall back to a documented-default-duration run (8s, ~$0.40) after an explicit remaining-budget check confirming `remaining ≥ quote + $0.20` headroom. No blind signing at any step — the quote is inspected before every payment signature this PROP exercises. | REQ-130–136 |
...
| `video` | **Quote-gated procedure (decided rule — see PROP-115):** request `xai/grok-imagine-video` (cheapest model) with `--duration-seconds 1` (schema-valid, REQ-130) and `--resolution 360p`. The manual x402 flow surfaces the exact price in the 402 response BEFORE signing (REQ-135). Inspect the quote: if `≤ $0.10`, sign and proceed — this is the primary path and answers the earlier "untested against upstream" concern, since the quote itself (not a guess) gates the spend. If the quote is `> $0.10` (upstream enforces a higher floor for 1s than the per-second table implies), do NOT sign; record the quote verbatim in `VERIFICATION.md`; only then fall back to the documented-default-duration run (8s) — but ONLY after an explicit check that `remaining budget ≥ quote + $0.20` headroom. No step signs a payment without having first inspected its exact quoted amount. | ≤$0.10 (primary path) or ~$0.40 (documented-default fallback, only if the primary quote exceeds $0.10 AND headroom check passes) | quote-gated — never a blind real-money action |
...
2. **CLI argument syntax for nested JSON (REQ-004).** This spec specifies `--param-json`/`--param @file.json` as the mechanism for structured input without picking a specific CLI-parsing library (yargs/commander/citty/etc.); the concrete library choice is an implementation decision for Phase 2 (TDD/impl), not a spec-level requirement, since no functional REQ depends on which library is used as long as the flag contract above holds.
