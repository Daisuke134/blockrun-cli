...
| `src/index.ts` (entrypoint) | registers all 18 commands, `--help`, top-level dispatch | process argv, process.exit |
...
|---|---|---|---|
| PROP-001 | REQ-001, REQ-002 | 0 | `package.json` has `bin.blockrun`; `src/index.ts` registers exactly the 18 names (assert via a Tier-1 test importing the command registry array and checking `.length === 18` and the name set matches the fixture). |
